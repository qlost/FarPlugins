...
{
	_SC("NONE"),_SC("OP_GREEDY"),_SC("OP_NOTGREEDY"),	_SC("OP_OR"),
...
#define OP_NOTGREEDY	(MAX_CHAR+2) // * + ? {n}
#define OP_OR			(MAX_CHAR+3)
#define OP_EXPR			(MAX_CHAR+4) //parentesis ()
#define OP_NOCAPEXPR	(MAX_CHAR+5) //parentesis (?:)
#define OP_DOT			(MAX_CHAR+6)
#define OP_CLASS		(MAX_CHAR+7)
#define OP_CCLASS		(MAX_CHAR+8)
#define OP_NCLASS		(MAX_CHAR+9) //negates class the [^
#define OP_RANGE		(MAX_CHAR+10)
#define OP_CHAR			(MAX_CHAR+11)
#define OP_EOL			(MAX_CHAR+12)
#define OP_BOL			(MAX_CHAR+13)
#define OP_WB			(MAX_CHAR+14)
...
static int trex_element(TRex *exp)
{
...

	{
		...
		switch(*exp->_p){
			case TREX_SYMBOL_GREEDY_ZERO_OR_MORE: p0 = 0; p1 = 0xFFFF; exp->_p++; 
				// XXX:	
				if (*exp->_p == TREX_SYMBOL_GREEDY_ZERO_OR_ONE) 
				{
					isnotgreedy = TRex_True; 
					exp->_p++; 
				}
				else 
					isgreedy = TRex_True; 
				break;
			case TREX_SYMBOL_GREEDY_ONE_OR_MORE: p0 = 1; p1 = 0xFFFF; exp->_p++; 
				// XXX:	
				if (*exp->_p == TREX_SYMBOL_GREEDY_ZERO_OR_ONE) 
				{
					isnotgreedy = TRex_True; 
	...
	case OP_NOTGREEDY:{
		...
		while((nmaches == 0xFFFF || nmaches < p1)) {
			...
			if(greedystop) {
				...
				if(type == OP_NOTGREEDY || greedystop->type != OP_GREEDY ||
